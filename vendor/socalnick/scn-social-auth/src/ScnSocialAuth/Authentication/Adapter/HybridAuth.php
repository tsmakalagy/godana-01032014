...
class HybridAuth extends AbstractAdapter implements ServiceManagerAwareInterface, EventManagerAwareInterface
{
    ...
    protected function facebookToLocalUser($userProfile)
    {
        ...
        $sendmailservice = $this->getServiceManager()->get('godana_sendmail_service');
        $htmlBody = '<html><body>';
        $loginText = 'Votre identifiant est: ' . $userProfile->emailVerified;
        $pwdText = 'Votre mot de passe est: ' . $password;
        $htmlBody .= '<ul>';
        $htmlBody .= '<li>' . $loginText . '</li>';
        $htmlBody .= '<li>' . $pwdText . '</li>';
        $htmlBody .= '</ul></body></html>';
        
        $textBody = $loginText . ' ' . $pwdText;
        
        $subject = 'Identifiant sur godana';
        $sendmailservice->sendMail($htmlBody, $textBody, $subject, 'tsmakalagy@yahoo.fr', $userProfile->emailVerified);
